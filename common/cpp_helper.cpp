...
__attribute__((no_profile_instrument_function, noinline))
extern "C" void bb_hook(uint64_t bb_inst, uint64_t bb_id, uint64_t threshold) {
    if (ifStart) {
        if (wait) {
            omp_set_lock(&lock);
            omp_unset_lock(&lock);
...
__attribute__((no_profile_instrument_function, noinline))
extern "C" void init_arrays(uint64_t num_bbs) {
...
extern "C" void roi_begin_() {
    ...
    std::string outputfile = "all_output_" + std::to_string(omp_get_max_threads()) + "_threads.txt";
