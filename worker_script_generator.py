...
for bench in benchmarks:
    ...
    for rid in rep_data.values():
        ...
        for papi_cmd in papi_cmds:
            arm_file_name = Path(rid_dir/)
            arm_runscript += f"cd {rid_dir.as_posix()} && {papi_cmd} && ./{bench}_papi_{rid};\n"
            x86_runscript += f"cd {rid_dir.as_posix()}/x86_64 && {papi_cmd} && ./{bench}_papi_{rid};\n"

with open(f"run_all_arm_{target_k}_papi_point.sh", "w") as f:
    f.write(arm_runscript)

with open(f"run_all_x86_{target_k}_papi_point.sh", "w") as f:
    f.write(arm_runscript)
